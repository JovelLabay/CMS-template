...
const storeZones = async (data: any) => {
    try {
        ...
        for (let i = 0; i < zonesData.length; i++) {
            ...
            // Else, insert
            await prisma.zones.findUnique({ where: { kpx_zone_id: zonesData[i].zoneId } }).then(
                async (result) => {
                    if (result === null) {
                        await prisma.zones.create({ data: { ...objZonesData } });
                    } else {
                        await prisma.zones.update({ where: { kpx_zone_id: zonesData[i].zoneId }, data: { ...objZonesData } });
                    }
                }
            );
        }
...
const storeRegions = async (data: any) => {
    try {
        ...
        for (let i = 0; i < regionsData.length; i++) {
            ...
            // Else, insert
            await prisma.regions.findUnique({ where: { kpx_region_id: regionsData[i].regionId } }).then(
                async (result) => {
                    if (result === null) {
                        await prisma.regions.create({ data: { ...objRegionsData } });
                    } else {
                        await prisma.regions.update({ where: { kpx_region_id: regionsData[i].regionId }, data: { ...objRegionsData } });
                    }
                }
            );
        }
...
const storeAreas = async (data: any) => {
    try {
        ...
        for (let i = 0; i < areasData.length; i++) {
            ...
            // Else, insert
            await prisma.areas.findUnique({ where: { kpx_area_id: areasData[i].areaId } }).then(
                async (result) => {
                    if (result === null) {
                        await prisma.areas.create({ data: { ...objAreasData } });
                    } else {
                        await prisma.areas.update({ where: { kpx_area_id: areasData[i].areaId }, data: { ...objAreasData } });
                    }
                }
            );
        }
...
const storeBranches = async (data: any) => {
    try {
        ...
        for (let i = 0; i < branchData.length; i++) {
            ...
            // Else, insert
            await prisma.branches.findUnique({ where: { kpx_branch_id: branchData[i].branchId } }).then(
                async (result) => {
                    if (result === null) {
                        await prisma.branches.create({ data: { ...objBranchesData } });
                    } else {
                        await prisma.branches.update({ where: { kpx_branch_id: branchData[i].branchId }, data: { ...objBranchesData } });
                    }
                }
            );
        }
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        Promise.all(endpoints.map((endpoint) => axios.get(endpoint, { headers: headerOpts })))
            .then(
